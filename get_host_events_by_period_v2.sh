...
# Конвертируем период в секунды с ограничением в 7 дней
...
# Время начала в формате timestamp
...
# Выводим информацию о событиях в удобочитаемом виде
echo "$events_response" | jq '.result[] | {
  "event_id": .eventid,
  "name": .name,
  "start_time": (.clock | tonumber | . + 10800 | strftime("%Y-%m-%dT%H:%M:%S %Z")),  # Преобразование в МСК
  "severity": .severity,
